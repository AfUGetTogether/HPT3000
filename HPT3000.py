...
def compute_hp_locations(
    ...
    if use_roof_layer and allow_roof_fallback and roofs_gdf is not None and roof_type_col and roof_type_col in roofs_gdf.columns and roof_allowed_values:
        ...
        # Schritt 3: optional Gebäude- & Leistungsinfo per centroid-within
        roofs_filtered["__centroid2"] = roofs_filtered.geometry.centroid
        join_bld = gpd.sjoin(
            roofs_filtered.set_geometry("__centroid2")[["__centroid2"]],
            b_with_flurstk[[building_id_col, flurstk_id_col, power_col]] if power_col_present else b_with_flurstk[[building_id_col, flurstk_id_col, "geometry"]],
            how="left",
            predicate="within"
        )
        # mapping: roof_idx -> (flurstk_id, building_id, power)
        for ridx, r in roofs_filtered.iterrows():
            geom = r.geometry
            if geom is None or (hasattr(geom, "is_empty") and r.geometry.is_empty):
                continue
